...
We note that the hexagons with the highest number of visits are near Wil, SG and Zurich Zentrum, which is where X resides (home and WG). St. Gallen also has a higher number of visits which is X's workplace. The university campus Wädenswil and Irchel do not show a notably higher number of visits.
...
The Wädenswil Campus has the highest number of visits (dark red), followed by Zurich Center, Irchel Campus and Kilchberg, and Wiesendangen (red). The orange cells are individually spread all throughout, likely indicating places of residence for an individual.
...
In the heatmap, Zurich Center and Wädenswil are the most visited locations. There is also a rather high density north of the Zürichsee between Meilen, Rapperswil and Wetzikon.

```{python}
raster_based_intersection_group.map_locations(column='num_visits')
```

Wädenswil Campus has the most visits, followed by Wollishofen, Kilchberg. The Irchel Campus is the next most visited cell, then followed by Zurich Center. 
```{python}
